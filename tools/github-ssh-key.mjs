...
const privateKeyPath = join(directory, "github_neot");
const publicKeyPath = `${privateKeyPath}.pub`;
const comment = options.comment ?? `neot-server@${hostname()}`;
...
function printHelp() {
  output.log(`Generate a temporary ED25519 keypair for NEOT server access to GitHub.

Usage:
  npm run github:ssh-key
  npm run github:ssh-key -- --comment "neot-server"
